...
public class FloatEdit : MonoBehaviour, IEdibleParameterUI
{
    ...
    {
        pendingValue = Mathf.Lerp(param.min, param.max, value);
        inputField.text = "" + Math.Round(pendingValue, 2);
    ...
    public IEdibleParameterUI Set(EditableParam editable)
    {
        ...
        originalValue = (float)field.GetValue(param.component);
    ...
    public bool IsSuitableValue(float v) => v >= param.min && v <= param.max;

    public int GetEditCost() => Mathf.CeilToInt(Mathf.Abs(originalValue - pendingValue) * param.cost);
